...
public static class EndlessGridPatcher
{
    ...
    {
        var seeded = CheatsManager.KeepCheatsEnabled
                  && PrefsManager.Instance.GetBool($"cheat.{SeedOverrideCheat.IDENTIFIER}");
        RandomManager.Initialize(seeded);
    }
    ...
    private static int Random_Range_Int(int min, int max, string methodName) =>
        RandomManager.RandomInstance.Range(min, max);
